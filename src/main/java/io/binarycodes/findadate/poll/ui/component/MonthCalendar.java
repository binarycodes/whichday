...
public class MonthCalendar extends CustomField<Set<LocalDate>> {
    ...
    public MonthCalendar(LocalDate today, String previousLabel, String nextLabel) {
        ...
        var previous = new Button(new Icon(VaadinIcon.ANGLE_LEFT), ignored -> showMonth(visibleMonth.minusMonths(1)));
        previous.setAriaLabel(previousLabel);
        previous.addClassName("action-link");
        var next = new Button(new Icon(VaadinIcon.ANGLE_RIGHT), ignored -> showMonth(visibleMonth.plusMonths(1)));
        next.setAriaLabel(nextLabel);
        next.addClassName("action-link");
    ...
    /** The grid starts on whatever the locale calls the first day of the week. */
    private LocalDate firstCellOf(YearMonth month) {
        var firstOfMonth = month.atDay(1);
        var firstWeekday = WeekFields.of(locale()).getFirstDayOfWeek();
        var offset = (firstOfMonth.getDayOfWeek().getValue() - firstWeekday.getValue() + WEEK_LENGTH) % WEEK_LENGTH;
        return firstOfMonth.minusDays(offset);
    }

    private List<DayOfWeek> orderedWeekdays() {
        var first = WeekFields.of(locale()).getFirstDayOfWeek();
        return IntStream.range(0, WEEK_LENGTH)
                .mapToObj(offset -> first.plus(offset))
                .toList();
    }
